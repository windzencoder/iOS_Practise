...
@interface WZSwipableViewController ()  <UIScrollViewDelegate>
...
@implementation WZSwipableViewController
...
- (instancetype)initWithTitle:(NSString *)title andSubTitles:(NSArray *)subTitles andControllers:(NSArray *)controllers underTabbar:(BOOL)underTabbar
{
    ...
    if (self) {
        ...
        _titleBar = [[WZTitleBarView alloc] initWithFrame:CGRectMake(0, 0, self.view.bounds.size.width, titleBarHeight) andTitles:subTitles];
        _titleBar.backgroundColor = [UIColor clearColor];
        [self.view addSubview:_titleBar];
        
        //水平滚动的vc
        _viewPager = [[WZHorizonalTableViewController alloc] initWithViewControllers:controllers];
        
        CGFloat height = CGRectGetHeight(self.view.bounds) - titleBarHeight - 64 - (underTabbar ? 49 : 0);
        _viewPager.view.frame = CGRectMake(0, titleBarHeight, CGRectGetWidth(self.view.bounds), height);
        ...
        __weak WZTitleBarView *weakTitleBar = _titleBar;
        __weak WZHorizonalTableViewController *weakViewPager = _viewPager;
